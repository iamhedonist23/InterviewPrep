...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return item
    ? {
        ...
        description:
          item.description ??
          `Practice ${item.name} interview questions with useful answer guidance.`,
    ...
    <section className="py-16 sm:py-20">
      ...
      <Container>
        ...
        {item.subcategories.length > 0 && (
          <div className="mt-14">
            <h2 className="font-display text-2xl font-bold">Explore topics</h2>
            <div className="mt-5 flex flex-wrap gap-3">
              {item.subcategories.map((subcategory) => (
                <Link
                  key={subcategory.id}
                  href={`/interview-questions?category=${item.slug}&subcategory=${subcategory.slug}`}
