...
export default async function Home() {
  ...
  const startingPointCards = [
    { title: "I have an interview soon", text: "Pick the role, review the most useful questions, and work through timed practice so you can sharpen your weak spots.", primaryHref: "/practice", primaryLabel: "Start practicing", secondaryHref: "/mock-interview", secondaryLabel: "Take a mock interview" },
    { title: "I want to learn", text: "Follow a topic from beginner to advanced, then close the loop by practicing the related questions and scenarios.", primaryHref: "/learn", primaryLabel: "Explore learning paths", secondaryHref: "/interview-questions", secondaryLabel: "Browse questions" },
    { title: "I want to practice", text: "Use filters for experience, difficulty, and interview type to work on exactly the questions you need right now.", primaryHref: "/interview-questions", primaryLabel: "Browse the library", secondaryHref: "/search", secondaryLabel: "Search questions" },
  ];

  return <>
  {homepageFaq && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(homepageFaq) }} />}
  <section className="overflow-hidden border-b border-ink/10"><Container className="grid min-h-[610px] items-center gap-12 py-20 lg:grid-cols-[1.05fr_.95fr] lg:py-24"><div><Badge>100% free, always</Badge><h1 className="mt-6 max-w-3xl font-display text-5xl font-bold leading-[1.02] tracking-tight sm:text-7xl">Interview Questions &amp; Answers for Every Career</h1><p className="mt-4 max-w-xl text-lg leading-8 text-coral">Prepare smarter. Interview with confidence.</p><p className="mt-7 max-w-xl text-lg leading-8 text-ink/65">Explore practical interview questions, detailed answers, learning paths, guided practice, and mock interviews for technical and behavioral interviews.</p><p className="mt-3 max-w-xl leading-7 text-ink/60">Choose a role, technology, or experience level and move from understanding to practice with confidence.</p><div className="mt-9 flex flex-wrap gap-3"><Button href="/practice">Start practicing <ArrowRight size={17} /></Button><Button href="/mock-interview" variant="outline">Take a mock interview</Button></div><div className="mt-8 flex flex-wrap gap-2">{levels.map(level => <Link key={level.value} href={`/interview-questions?experience=${level.value}`} className="rounded-full border border-ink/15 bg-paper px-3 py-2 text-sm font-semibold text-ink/70 transition hover:border-coral hover:text-coral">{level.label}</Link>)}</div></div><div className="relative"><div className="absolute -inset-5 rounded-[3rem] bg-mint/60 blur-2xl" /><Card className="relative rotate-1 bg-white p-7 shadow-xl shadow-ink/10"><div className="flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-widest text-ink/50">Today&apos;s practice</span><span className="text-sm font-bold text-coral">03 / 10</span></div><div className="mt-5 h-2 rounded-full bg-mint"><div className="h-full w-[30%] rounded-full bg-coral" /></div><p className="mt-10 font-display text-2xl font-bold leading-tight">Tell me about a project you&apos;re proud of.</p><p className="mt-5 text-sm leading-6 text-ink/55">A strong answer connects your actions to a clear outcome. Keep your story focused and specific.</p><div className="mt-8 rounded-xl border border-ink/10 bg-paper p-4 text-sm text-ink/60">Your answer appears here...</div><div className="mt-5 flex justify-end"><Button href="/practice">Next question <ChevronRight size={17} /></Button></div></Card></div></Container></section>
  ...
  <Section eyebrow="Choose your starting point" title="Find the right next step">
    <p className="max-w-2xl text-lg leading-8 text-ink/60">Whether you are preparing for a role, a technology, or a specific interview type, start with the path that matches your goal.</p>
    <div className="mt-8 grid gap-4 lg:grid-cols-3">{startingPointCards.map((card) => <div key={card.title} className="flex h-full flex-col rounded-2xl border border-ink/10 bg-white/70 p-5"><p className="text-xs font-bold uppercase tracking-[.18em] text-coral">{card.title}</p><p className="mt-4 text-sm leading-7 text-ink/60">{card.text}</p><div className="mt-5 flex flex-wrap gap-2"><Button href={card.primaryHref}>{card.primaryLabel}</Button><Button href={card.secondaryHref} variant="outline">{card.secondaryLabel}</Button></div></div>)}</div>
    <div className="mt-10">
      <p className="text-sm font-bold uppercase tracking-[.18em] text-ink/50">Popular preparation paths</p>
      <div className="mt-4 flex flex-wrap gap-2">{categories.slice(0, 12).map((category) => <Link key={category.slug} href={`/interview-questions?category=${category.slug}`} className="rounded-full border border-ink/15 bg-paper px-3 py-2 text-sm font-semibold text-ink/70 transition hover:border-coral hover:text-coral">{category.name}</Link>)}</div>
    </div>
  </Section>
