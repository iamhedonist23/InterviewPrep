...
import { studyModuleSchema } from "@/lib/study";
import { invalidateLearnCache } from "@/lib/study-public";
import { createStudyModule } from "@/lib/study";
...
export async function GET() {
  const modules = await prisma.studyModule.findMany({
    orderBy: [{ studyPath: { sortOrder: "asc" } }, { sortOrder: "asc" }],
    select: {
      id: true,
      title: true,
      studyPath: { select: { categoryId: true, name: true } },
    },
  });
...
export async function POST(request: Request) {
  ...
  try {
    const data = studyModuleSchema.parse(body);
    const result = await createStudyModule(data);
...
export async function PATCH(request: Request) {
  ...
  try {
    const result = await prisma.studyModule.update({
      where: { id },
      data: {
