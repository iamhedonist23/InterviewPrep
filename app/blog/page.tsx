...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description:
      "Practical, original guides for preparing for interviews with more clarity.",
...
export default async function BlogPage({ searchParams }: Props) {
  ...
  if (!Number.isInteger(rawPage) || rawPage < 1) notFound();
  const result = await listArticles(
    rawPage,
    category,
  );
  if (rawPage > result.pageCount) notFound();
  const pageUrl = (page: number) =>
    `/blog?${new URLSearchParams({ ...(category ? { category } : {}), page: String(page) })}`;
  return (
    <section className="py-16 sm:py-20">
      <Container>
        ...
        <h1 className="mt-3 font-display text-4xl font-bold sm:text-5xl">
          Practical guidance for the next conversation
        </h1>
        <p className="mt-4 max-w-2xl text-lg leading-8 text-ink/60">
          Original notes on preparation, communication, and the decisions that
          make interviews clearer.
        </p>
        <div className="mt-10 flex flex-wrap gap-3">
          <Link
            href="/blog"
            className={`rounded-full border px-4 py-2 text-sm font-bold ${!category ? "border-coral text-coral" : "border-ink/15"}`}
          >
            All resources
          </Link>
          {["General", "Technology"].map((item) => (
            <Link
        ...
        {result.articles.length ? (
          <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {result.articles.map((article) => (
              ...
              >
                ...
                <Link
                  href={`/blog/${article.slug}`}
                  className="mt-6 inline-block text-sm font-bold text-coral"
                >
                  Read guide
                </Link>
              </article>
          ...
          <div className="mt-10 rounded-2xl border border-dashed border-ink/20 p-12 text-center text-ink/60">
