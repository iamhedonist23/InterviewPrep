...
export const metadata: Metadata = {
  ...
  description:
    "Practice interview questions for free, choose a category and difficulty, and build confidence one question at a time.",
...
export default async function PracticePage({ searchParams }: Props) {
  ...
  const categories = (await getCachedPublicCategories()).map(({ id, name, slug }) => ({
    id,
    name,
    slug,
  }));
      ...
      <Container>
        <div className="mx-auto max-w-3xl">
          ...
          <h1 className="mt-3 font-display text-4xl font-bold sm:text-5xl">
            Practice with a little structure.
          </h1>
          <p className="mt-4 max-w-2xl text-lg leading-8 text-ink/60">
            Choose a focus, write your answer, and build confidence one question at a time.
            You can practice without an account.
          </p>
          <div className="mt-10">
            <PracticeSetup categories={categories} initialQuestionSlug={question} />
          </div>
        </div>
      </Container>
    </section>
