...
// Every optional text field in a resume can arrive as `undefined` (field
// omitted), `null` (stored-but-empty in the DB, since that's what Prisma
// returns for unset nullable columns), or `""` (cleared in the UI). All
// three should be treated identically as "no value".
const nullableToEmpty = z.preprocess((value) => (value === null || value === undefined ? "" : value), z.string());

const optionalUrl = nullableToEmpty
  .pipe(
    z
      .string()
      .trim()
...
const optionalEmail = nullableToEmpty
  .pipe(
    z
      .string()
      .trim()
...
const shortText = (max: number) => nullableToEmpty.pipe(z.string().trim().max(max)).optional();
