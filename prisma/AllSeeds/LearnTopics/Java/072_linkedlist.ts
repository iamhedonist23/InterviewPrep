...
/**
 * DB-ready Java topic seed: 72/356
 *
...
export const topic: TopicSeed = {
  title: "LinkedList",
  slug: "linkedlist",
  description: "`LinkedList` is taught as a Java concept through its language/runtime behavior, practical use, and the edge cases that matter when the code is changed or used in production.",
  estimatedMinutes: 20,
  sections: [
    {
    ...
    },
    {
      ...
      content: "In production, choose `LinkedList` based on actual access patterns rather than habit. Check expected time complexity, memory overhead, ordering requirements, equality/comparison behavior, and whether multiple threads will access the structure.\\n\\nBe especially careful when mutable objects participate in hashing or ordering. A collection can remain internally valid while lookups become surprising because the state used to locate an element has changed."
    },
    {
      title: "Example",
      content: "Deque<String> queue = new LinkedList<>();\\nqueue.addLast(\\\"A\\\");\\nqueue.addLast(\\\"B\\\");\\n\\nSystem.out.println(queue.removeFirst()); // A"
    },
    {
      title: "Practice",
      content: "Build a small program demonstrating `LinkedList` with the operation it is designed for. Include one boundary case such as a duplicate, missing key, ordering change, or mutation. Explain the observed result from the collection's contract."
    }
  ],
};
