...
async function seed() {
  ...
  const topic: TopicSeed = {
    ...
    description: "Detailed, human-oriented explanation of Java security architecture. Covers the Java rule or API contract, concrete code, runtime behavior, edge cases, common misconceptions, production considerations, and interview preparation.",
    estimatedMinutes: 20,
    sections: [{"title": "What it is and why it matters", "content": "Java security architecture is a Java security concept/API where correctness includes both the Java API contract and the security properties of the surrounding protocol or storage design. The safe explanation must distinguish confidentiality, integrity, authenticity, randomness, key management, and trust."}, {"title": "A concrete Java example", "content": "MessageDigest digest = MessageDigest.getInstance(\"SHA-256\");\nbyte[] hash = digest.digest(\"hello\".getBytes(StandardCharsets.UTF_8));"}, {"title": "How the mechanism behaves", "content": "For Java security architecture, identify the security primitive or trust decision involved and explain what it does not provide. A hash is not encryption; a digital signature is not confidentiality; a truststore is not a password store; and a secure random generator is not interchangeable with a general-purpose PRNG."}, {"title": "Edge cases and common mistakes", "content": "Avoid inventing algorithms, modes, key sizes, or protocol defaults as universal best practice. Never store plaintext passwords, never build SQL with untrusted string concatenation, and never deserialize untrusted Java object streams without a deliberate security design."}, {"title": "Using it in production", "content": "Use standard JDK APIs with current provider support, explicit algorithms where appropriate, secure key storage, certificate validation, secret rotation, and dependency/runtime updates. Security claims should be reviewed against current platform guidance."}, {"title": "Interview-ready explanation", "content": "A strong interview answer for Java security architecture should start with the precise definition, identify the Java rule or API contract, explain the example, and then mention one edge case and one trade-off. If asked a follow-up, explain the condition under which the original statement changes."}, {"title": "Practice questions", "content": "1. What security property does Java security architecture provide?\n2. What does it not provide?\n3. What is the main misuse?\n4. How would you deploy it safely?"}],
  };
