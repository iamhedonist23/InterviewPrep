...
async function seed() {
  ...
  const topic: TopicSeed = {
    title: "jstat",
    slug: "jstat",
    description: "Detailed, human-oriented explanation of jstat. Covers the Java rule or API contract, concrete code, runtime behavior, edge cases, common misconceptions, production considerations, and interview preparation.",
    estimatedMinutes: 20,
    sections: [{"title": "What it is and why it matters", "content": "jstat is a JVM/runtime or performance concept. The useful explanation connects the Java source code to observable runtime behavior while separating specification guarantees from HotSpot-specific implementation details."}, {"title": "A concrete Java example", "content": "// Measure or inspect the behavior rather than guessing.\nSystem.out.println(Runtime.getRuntime().availableProcessors());"}, {"title": "How the mechanism behaves", "content": "For jstat, identify what the JVM actually does, what can be observed with standard diagnostics, and which parts depend on the selected JDK/JVM and runtime configuration. Performance claims must include workload and measurement conditions."}, {"title": "Edge cases and common mistakes", "content": "Do not present a HotSpot implementation detail as a universal Java rule. Avoid microbenchmark conclusions from a single timing run; JIT compilation, warm-up, dead-code elimination, allocation behavior, and GC can distort naive measurements."}, {"title": "Using it in production", "content": "Use jstat as part of evidence-driven diagnosis. Capture the symptom, establish a baseline, inspect the relevant runtime signal, change one variable, and measure again."}, {"title": "Interview-ready explanation", "content": "A strong interview answer for jstat should start with the precise definition, identify the Java rule or API contract, explain the example, and then mention one edge case and one trade-off. If asked a follow-up, explain the condition under which the original statement changes."}, {"title": "Practice questions", "content": "1. What is jstat?\n2. What is guaranteed by Java versus HotSpot?\n3. What metric or tool would you inspect?\n4. What common measurement mistake should you avoid?"}],
  };
