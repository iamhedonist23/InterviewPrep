...
async function seed() {
  ...
  const topic: TopicSeed = {
    title: "Set interface",
    slug: "set-interface",
    description: "Detailed, human-oriented explanation of Set interface. Covers the Java rule or API contract, concrete code, runtime behavior, edge cases, common misconceptions, production considerations, and interview preparation.",
    estimatedMinutes: 20,
    sections: [{"title": "What it is and why it matters", "content": "Set interface is a Java concept with a defined language, API, or runtime contract. Learn the exact construct involved, what Java guarantees, how the operation behaves for normal and boundary inputs, and which implementation details must not be mistaken for language guarantees."}, {"title": "A concrete Java example", "content": "class Account {\n    private final String id;\n    Account(String id) { this.id = id; }\n    String id() { return id; }\n}\n\nAccount account = new Account(\"A-100\");\nSystem.out.println(account.id());"}, {"title": "How the mechanism behaves", "content": "With Set interface, separate compile-time rules from runtime behavior. Identify the inputs, outputs, state changes, exceptions, ordering, and lifecycle involved. If a detail depends on a particular JDK implementation, JVM, operating system, or configuration, state that explicitly."}, {"title": "Edge cases and common mistakes", "content": "The main interview trap for Set interface is replacing the actual contract with a slogan. Explain the preconditions, observable result, failure mode, and one edge case. Do not claim that a feature is always faster, safer, immutable, thread-safe, or O(1) without conditions."}, {"title": "Using it in production", "content": "Use Set interface when it makes the required behavior clear and correct. In production, review lifecycle, error handling, concurrency, performance, compatibility, and observability relevant to the concept."}, {"title": "Interview-ready explanation", "content": "A strong interview answer for Set interface should start with the precise definition, identify the Java rule or API contract, explain the example, and then mention one edge case and one trade-off. If asked a follow-up, explain the condition under which the original statement changes."}, {"title": "Practice questions", "content": "1. Define Set interface precisely.\n2. What Java rule or API contract controls it?\n3. Walk through the example.\n4. What is an important edge case?\n5. What is a common misconception?\n6. What would you verify before using it in production?"}],
  };
