...
async function seed() {
  ...
  const topic: TopicSeed = {
    ...
    description: "Detailed, human-oriented explanation of Reference types. Covers the Java rule or API contract, concrete code, runtime behavior, edge cases, common misconceptions, production considerations, and interview preparation.",
    estimatedMinutes: 20,
    sections: [{"title": "What it is and why it matters", "content": "A reference variable holds a reference value that identifies an object or is null; it does not contain the object itself. Classes, interfaces, arrays, enums, records, and other object types are reference types. Java still passes the reference value by value when an object is supplied to a method."}, {"title": "Concrete Java example", "content": "List<String> names = new ArrayList<>();\nnames.add(\"Asha\");\nList<String> alias = names;\nalias.add(\"Ravi\");\nSystem.out.println(names.size()); // 2"}, {"title": "How Java actually behaves", "content": "Two variables can refer to the same object. Reassigning one reference does not reassign the other, but mutating the shared object is visible through both references. A null reference is not an object and dereferencing it causes NullPointerException."}, {"title": "Edge cases and interview traps", "content": "Do not say the variable \"contains the object\" or that Java passes objects by reference. The precise statement is that a copy of the reference value is passed. Also avoid claiming a universal stack/heap placement for every reference; JVM implementations are free to optimize representation and allocation."}, {"title": "Production guidance", "content": "Use immutable value objects where shared mutable state would make ownership unclear. Document whether an API keeps, copies, or mutates supplied mutable objects."}, {"title": "Interview-ready explanation", "content": "A strong answer for Reference types should define the concept, state the Java rule or API contract, walk through the example, and finish with the most important limitation or edge case. Avoid broad claims that are not guaranteed by the Java specification or API."}, {"title": "Practice questions", "content": "1. Define Reference types precisely.\n2. What does Java or the JDK guarantee?\n3. Walk through the example.\n4. What is an important edge case?\n5. What is a common production misuse?"}],
  };
