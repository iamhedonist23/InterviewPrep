...
async function seed() {
  ...
  const topic: TopicSeed = {
    title: "HashMap",
    slug: "hashmap",
    description: "Detailed, human-oriented explanation of HashMap. Covers the Java rule or API contract, concrete code, runtime behavior, edge cases, common misconceptions, production considerations, and interview preparation.",
    estimatedMinutes: 20,
    sections: [{"title": "What it is and why it matters", "content": "HashMap stores key-value mappings using hashing. It uses a key’s hash code to locate a bucket and equality checks to distinguish keys that land in the same bucket. Average lookup is expected O(1), but that is not a worst-case guarantee."}, {"title": "Concrete Java example", "content": "Map<String, Integer> counts = new HashMap<>();\ncounts.merge(\"java\", 1, Integer::sum);\ncounts.merge(\"java\", 1, Integer::sum);\nSystem.out.println(counts.get(\"java\")); // 2"}, {"title": "How Java actually behaves", "content": "Correct key behavior depends on a stable equals/hashCode contract. Modern JDK implementations can use tree bins for heavily collided buckets, but the exact internal thresholds and structure are implementation details, not API guarantees. Resizing redistributes entries when the table grows beyond its threshold."}, {"title": "Edge cases and interview traps", "content": "Mutable keys are dangerous: if a key changes in a way that affects hashCode or equals after insertion, a later lookup may not find the entry. HashMap also permits one null key and multiple null values."}, {"title": "Production guidance", "content": "Use immutable key types. Size maps with a realistic initial capacity when appropriate, and do not assume iteration order; use LinkedHashMap when insertion/access order is part of the requirement."}, {"title": "Interview-ready explanation", "content": "A strong answer for HashMap should define the concept, state the Java rule or API contract, walk through the example, and finish with the most important limitation or edge case. Avoid broad claims that are not guaranteed by the Java specification or API."}, {"title": "Practice questions", "content": "1. Define HashMap precisely.\n2. What does Java or the JDK guarantee?\n3. Walk through the example.\n4. What is an important edge case?\n5. What is a common production misuse?"}],
  };
