...
/**
 * DB-ready Java topic seed: 66/356
 *
...
export const topic: TopicSeed = {
  ...
  description: "`Checked vs unchecked exceptions` is taught as a Java concept through its language/runtime behavior, practical use, and the edge cases that matter when the code is changed or used in production.",
  estimatedMinutes: 20,
  sections: [
    {
      title: "Core explanation",
      content: "`Checked vs unchecked exceptions` belongs to Java's exception model and controls how abnormal conditions are represented, propagated, caught, or translated.\\n\\nTrace the call stack: where is the failure detected, which layer can recover, where should context be added, and where should the original cause be preserved? That is more useful than memorizing exception syntax."
    },
    {
      title: "Practical perspective",
      content: "For `Checked vs unchecked exceptions`, catch an exception only when the current layer can do something useful with it. Preserve causes when translating exceptions, and do not hide failures with empty catch blocks."
    },
    {
      title: "Example",
    ...
    {
      title: "Practice",
      content: "Create a small failure scenario for `Checked vs unchecked exceptions`. Trace the thrown exception, handler, cleanup, and final result. Then change the handler and explain how propagation changes."
    }
  ],
};
