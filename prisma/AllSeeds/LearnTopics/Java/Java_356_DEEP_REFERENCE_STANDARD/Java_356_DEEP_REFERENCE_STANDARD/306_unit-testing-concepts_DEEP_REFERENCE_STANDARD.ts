...
async function seed() {
  ...
  const module = await prisma.studyModule.upsert({
    where: { studyPathId_slug: { studyPathId: path.id, slug: "testing" } },
    update: {
      title: "Testing",
      description: "Deep, practical Java study covering testing.",
      isPublished: true,
      sortOrder: 0,
    },
    create: {
  ...
  const topic: TopicSeed = {
    ...
    slug: "unit-testing-concepts",
    description: "Deep, topic-specific explanation of Unit testing concepts, including the Java rule, how it behaves, practical usage, edge cases, common interview traps, and hands-on reasoning.",
    estimatedMinutes: 24,
    sections: [
      { title: "What the concept actually means", content: `Testing is evidence about observable behavior, not simply code execution. A useful test isolates a contract, controls important dependencies, and fails for the regression it is intended to catch.

The name alone is not the full definition. A strong understanding connects \`Unit testing concepts\` to the part of Java that gives it meaning: source-language rules, compiler type checking, library contracts, JVM execution, memory visibility, external I/O, or a design constraint. The same distinction matters in interviews because a definition without the governing rule usually breaks as soon as the interviewer changes the example.` },
      { title: "How it works", content: `Start by identifying the inputs, the Java rule that applies, and the observable result.

For \`Unit testing concepts\`, reason in this order:
...
Read the example by identifying the declared types, the operation being performed, and the point where Java applies the rule behind \`Unit testing concepts\`. Do not memorize the output alone. Explain why that output follows from the language or API contract.` },
      { title: "Important rules and edge cases", content: `Avoid accidental coupling to time, network, test order, global state, or implementation details unless those are intentionally under test.
...
Also check boundary cases such as null values, empty input, duplicate values, overflow or precision, invalid state, resource failure, concurrency, and API version differences whenever they are relevant to \`Unit testing concepts\`.` },
      { title: "Practical use in real Java code", content: `Use \`Unit testing concepts\` because it expresses a real requirement, not because the feature exists.

In production, evaluate readability, ownership, failure handling, compatibility, performance, and maintainability. If \`Unit testing concepts\` interacts with an external resource or another thread, include the failure and timing behavior in the design rather than documenting only the happy path.
...
**Mistake 3:** confusing similar concepts. When \`Unit testing concepts\` has a commonly confused neighbor, explicitly compare what is selected at compile time, what happens at runtime, and what contract each API provides.
...
**Mistake 4:** ignoring the failure path. Production-quality Java explanations include what happens when input is invalid, a resource is unavailable, or an operation cannot complete.` },
      { title: "Interview-ready explanation", content: `A concise but technically strong interview answer for \`Unit testing concepts\` should follow this structure:
...
For \`Unit testing concepts\`, avoid vague claims such as "Java handles it automatically" unless you can immediately explain what Java is actually doing.` },
      { title: "Practice and follow-up questions", content: `1. Define \`Unit testing concepts\` without using only its name as the definition.
...
5. What is one common misconception about \`Unit testing concepts\`?
