...
async function seed() {
  ...
  const module = await prisma.studyModule.upsert({
    where: { studyPathId_slug: { studyPathId: path.id, slug: "date-and-time" } },
    update: {
      title: "Date and Time",
      description: "Deep, practical Java study covering date and time.",
      isPublished: true,
      sortOrder: 0,
    },
    create: {
  ...
  const topic: TopicSeed = {
    title: "LocalDate and LocalTime",
    slug: "localdate-and-localtime",
    description: "Deep, topic-specific explanation of LocalDate and LocalTime, including the Java rule, how it behaves, practical usage, edge cases, common interview traps, and hands-on reasoning.",
    estimatedMinutes: 24,
    sections: [
      { title: "What the concept actually means", content: `\`LocalDate and LocalTime\` is a Java concept whose correct use depends on its language, library, compiler, or JVM contract. The important goal is to understand the rule, observe it in code, and know the boundary where the rule changes.

The name alone is not the full definition. A strong understanding connects \`LocalDate and LocalTime\` to the part of Java that gives it meaning: source-language rules, compiler type checking, library contracts, JVM execution, memory visibility, external I/O, or a design constraint. The same distinction matters in interviews because a definition without the governing rule usually breaks as soon as the interviewer changes the example.` },
      { title: "How it works", content: `Start by identifying the inputs, the Java rule that applies, and the observable result.

For \`LocalDate and LocalTime\`, reason in this order:
...
Read the example by identifying the declared types, the operation being performed, and the point where Java applies the rule behind \`LocalDate and LocalTime\`. Do not memorize the output alone. Explain why that output follows from the language or API contract.` },
      { title: "Important rules and edge cases", content: `For \`LocalDate and LocalTime\`, distinguish specification guarantees from implementation details, then consider readability, correctness, failure behavior, compatibility, and performance in the surrounding application.
...
Also check boundary cases such as null values, empty input, duplicate values, overflow or precision, invalid state, resource failure, concurrency, and API version differences whenever they are relevant to \`LocalDate and LocalTime\`.` },
      { title: "Practical use in real Java code", content: `Use \`LocalDate and LocalTime\` because it expresses a real requirement, not because the feature exists.

In production, evaluate readability, ownership, failure handling, compatibility, performance, and maintainability. If \`LocalDate and LocalTime\` interacts with an external resource or another thread, include the failure and timing behavior in the design rather than documenting only the happy path.
...
**Mistake 3:** confusing similar concepts. When \`LocalDate and LocalTime\` has a commonly confused neighbor, explicitly compare what is selected at compile time, what happens at runtime, and what contract each API provides.
...
**Mistake 4:** ignoring the failure path. Production-quality Java explanations include what happens when input is invalid, a resource is unavailable, or an operation cannot complete.` },
      { title: "Interview-ready explanation", content: `A concise but technically strong interview answer for \`LocalDate and LocalTime\` should follow this structure:
...
For \`LocalDate and LocalTime\`, avoid vague claims such as "Java handles it automatically" unless you can immediately explain what Java is actually doing.` },
      { title: "Practice and follow-up questions", content: `1. Define \`LocalDate and LocalTime\` without using only its name as the definition.
...
5. What is one common misconception about \`LocalDate and LocalTime\`?
