...
async function seed() {
  ...
  const topic: TopicSeed = {
    ...
    description: "Deep, topic-specific explanation of Checked vs unchecked exceptions, including the Java rule, how it behaves, practical usage, edge cases, common interview traps, and hands-on reasoning.",
    estimatedMinutes: 24,
    sections: [
      { title: "What the concept actually means", content: `Java exceptions model abnormal completion separately from ordinary return values. An exception can propagate up the call stack until a compatible handler is found.

The name alone is not the full definition. A strong understanding connects \`Checked vs unchecked exceptions\` to the part of Java that gives it meaning: source-language rules, compiler type checking, library contracts, JVM execution, memory visibility, external I/O, or a design constraint. The same distinction matters in interviews because a definition without the governing rule usually breaks as soon as the interviewer changes the example.` },
      { title: "How it works", content: `Start by identifying the inputs, the Java rule that applies, and the observable result.

For \`Checked vs unchecked exceptions\`, reason in this order:
...
try (BufferedReader reader = Files.newBufferedReader(Path.of("data.txt"))) {
    ...
    throw new UncheckedIOException("Unable to read data", e);
}
\`\`\`

Read the example by identifying the declared types, the operation being performed, and the point where Java applies the rule behind \`Checked vs unchecked exceptions\`. Do not memorize the output alone. Explain why that output follows from the language or API contract.` },
      { title: "Important rules and edge cases", content: `Checked-exception rules are compile-time constraints; RuntimeException and Error have different semantics. Cleanup and cause preservation are part of correct handling.
...
Also check boundary cases such as null values, empty input, duplicate values, overflow or precision, invalid state, resource failure, concurrency, and API version differences whenever they are relevant to \`Checked vs unchecked exceptions\`.` },
      { title: "Practical use in real Java code", content: `Use \`Checked vs unchecked exceptions\` because it expresses a real requirement, not because the feature exists.

In production, evaluate readability, ownership, failure handling, compatibility, performance, and maintainability. If \`Checked vs unchecked exceptions\` interacts with an external resource or another thread, include the failure and timing behavior in the design rather than documenting only the happy path.
...
**Mistake 3:** confusing similar concepts. When \`Checked vs unchecked exceptions\` has a commonly confused neighbor, explicitly compare what is selected at compile time, what happens at runtime, and what contract each API provides.
...
**Mistake 4:** ignoring the failure path. Production-quality Java explanations include what happens when input is invalid, a resource is unavailable, or an operation cannot complete.` },
      { title: "Interview-ready explanation", content: `A concise but technically strong interview answer for \`Checked vs unchecked exceptions\` should follow this structure:
...
For \`Checked vs unchecked exceptions\`, avoid vague claims such as "Java handles it automatically" unless you can immediately explain what Java is actually doing.` },
      { title: "Practice and follow-up questions", content: `1. Define \`Checked vs unchecked exceptions\` without using only its name as the definition.
...
5. What is one common misconception about \`Checked vs unchecked exceptions\`?
