...
async function seed() {
  ...
  const topic: TopicSeed = {
    title: "Constructors",
    slug: "constructors",
    description: "Deep, topic-specific explanation of Constructors, including the Java rule, how it behaves, practical usage, edge cases, common interview traps, and hands-on reasoning.",
    estimatedMinutes: 24,
    sections: [
...
Read the example by identifying the declared types, the operation being performed, and the point where Java applies the rule behind \`Constructors\`. Do not memorize the output alone. Explain why that output follows from the language or API contract.` },
...
Also check boundary cases such as null values, empty input, duplicate values, overflow or precision, invalid state, resource failure, concurrency, and API version differences whenever they are relevant to \`Constructors\`.` },
      { title: "Practical use in real Java code", content: `Use \`Constructors\` because it expresses a real requirement, not because the feature exists.

In production, evaluate readability, ownership, failure handling, compatibility, performance, and maintainability. If \`Constructors\` interacts with an external resource or another thread, include the failure and timing behavior in the design rather than documenting only the happy path.
...
**Mistake 3:** confusing similar concepts. When \`Constructors\` has a commonly confused neighbor, explicitly compare what is selected at compile time, what happens at runtime, and what contract each API provides.
...
**Mistake 4:** ignoring the failure path. Production-quality Java explanations include what happens when input is invalid, a resource is unavailable, or an operation cannot complete.` },
      { title: "Interview-ready explanation", content: `A concise but technically strong interview answer for \`Constructors\` should follow this structure:
...
For \`Constructors\`, avoid vague claims such as "Java handles it automatically" unless you can immediately explain what Java is actually doing.` },
      { title: "Practice and follow-up questions", content: `1. Define \`Constructors\` without using only its name as the definition.
...
5. What is one common misconception about \`Constructors\`?
