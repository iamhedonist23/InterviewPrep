...
/**
 * DB-ready Java topic seed: 172/356
 *
...
export const topic: TopicSeed = {
  title: "CyclicBarrier",
  slug: "cyclicbarrier",
  description: "`CyclicBarrier` is taught as a Java concept through its language/runtime behavior, practical use, and the edge cases that matter when the code is changed or used in production.",
  estimatedMinutes: 20,
  sections: [
    {
      title: "Core explanation",
      content: "`CyclicBarrier` addresses a specific part of Java concurrency: coordinating work or shared state when multiple threads can execute at overlapping times. The important question is what guarantee the feature provides—mutual exclusion, visibility, atomicity, task scheduling, coordination, cancellation, or a combination.\\n\\nStart by identifying the shared state and the race that could occur without coordination. Then identify the Java rule that prevents that race. This keeps concurrency reasoning grounded in guarantees rather than thread-count folklore."
    },
    {
      title: "Practical perspective",
      content: "Use `CyclicBarrier` only when its guarantee matches the problem. Consider interruption, cancellation, shutdown, contention, exception propagation, and whether the protected operation can block.\\n\\nA common production failure is solving one problem while creating another—for example, adding a lock for correctness and then holding it during slow I/O, which turns a correctness boundary into a throughput bottleneck."
    },
    {
      title: "Practice",
      content: "Create a two-thread example involving `CyclicBarrier`. Run it repeatedly, identify the shared state or coordination point, and explain what guarantee makes the corrected version safe. Then remove that guarantee and predict the failure mode."
    }
  ],
};
