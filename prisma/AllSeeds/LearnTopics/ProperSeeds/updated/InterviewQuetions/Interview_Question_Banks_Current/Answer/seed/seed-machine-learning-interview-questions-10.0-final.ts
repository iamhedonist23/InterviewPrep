...
async function main() {
  const category = await prisma.interviewCategory.upsert({
  ...
  for (const q of questions) {
    if (!subcategoryMap.has(q.subcategorySlug)) {
      ...
      const sub = await prisma.interviewSubcategory.upsert({
