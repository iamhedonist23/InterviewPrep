...
// Combine all topics
const topics = [...originalTopics, ...existingAdvancedTopics, ...newCategoryTopics, ...extraAdvancedTopics] as const;

// ---- Extended angles (14 angles) ----
const angles = [
  "for a small team",
...
For this ${categoryName} interview question, I would approach the problem ${angle}. The key is to understand the requirement before choosing a technology or implementation.
...
async function main() {
  ...
  for (let topicIndex = 0; topicIndex < topics.length; topicIndex++) {
    const [categoryName, baseQuestion, tag, explanation, sampleAnswer, detailedAnswer] = topics[topicIndex];
    ...
    for (let angleIndex = 0; angleIndex < angles.length; angleIndex++) {
      const question = `${baseQuestion} ${angles[angleIndex]}?`;
      const slug = `${slugify(baseQuestion)}-${slugify(angles[angleIndex])}`;
      
      // ---- Difficulty: make sure advanced topics & advanced angles produce more HARD questions ----
      let difficulty: Difficulty;
      // If the topic index is in the advanced range (>= originalTopics.length) or angle is advanced (>= 10), bias toward HARD
      if (topicIndex >= originalTopics.length || angleIndex >= 10) {
        difficulty = [Difficulty.MEDIUM, Difficulty.HARD, Difficulty.HARD][(topicIndex + angleIndex) % 3];
      } else {
        difficulty = [Difficulty.EASY, Difficulty.MEDIUM, Difficulty.HARD][(topicIndex + angleIndex) % 3];
      }

      const experienceLevel = [ExperienceLevel.FRESHER, ExperienceLevel.INTERNSHIP, ExperienceLevel.MID_LEVEL, ExperienceLevel.EXPERIENCED][(topicIndex + angleIndex) % 4];
      ...
      await prisma.interviewQuestion.upsert({
        ...
        update: {
          ...
          difficulty,
          interviewType,
          shortDescription: `A practical ${tag} question focused on ${angles[angleIndex]}.`,
          explanation,
          sampleAnswer,
          detailedAnswer: buildUltraDetailedAnswer(categoryName, baseQuestion, angles[angleIndex], tag, explanation, sampleAnswer),
          ...
          followUpQuestions: [`What would you measure after making that decision?`, `What would you change with more time?`],
          tags: [tag, slugify(categoryName)],
          isPublished: true,
          seoTitle: question,
          seoDescription: `Learn how to approach this ${tag} interview question with a clear, practical answer.`
        },
        create: {
          question,
          slug,
  ...
  console.log(`Seeded ${topics.length * angles.length} interview questions and ${articleSeeds.length} articles.`);
}
