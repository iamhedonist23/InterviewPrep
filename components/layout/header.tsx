...
export function Header() { const { data: session } = useSession(); return <header className="border-b border-ink/10 bg-paper/90 backdrop-blur"><Container className="relative flex h-20 items-center justify-between gap-6"><Link href="/" className="flex items-center gap-2 font-display text-xl font-bold tracking-tight"><Image src={logo} alt="InterviewPrep logo" width={40} height={40} className="rounded-full object-cover" priority /><span>Interview<span className="text-coral">Prep</span></span></Link><nav className="hidden items-center gap-5 lg:flex" aria-label="Primary navigation">{links.map(([label, href]) => <Link className="text-sm font-semibold text-ink/70 hover:text-coral" href={href} key={href}>{label}</Link>)}</nav><div className="hidden items-center gap-2 lg:flex"><Button href="/resume-builder" variant="outline">Create Resume</Button>{!session ? <Button href="/login">Log in</Button> : <Link href="/dashboard" aria-label="Go to your dashboard" title="Your dashboard" className="grid h-10 w-10 place-items-center rounded-full bg-mint hover:bg-coral hover:text-white"><CircleUserRound size={21} aria-hidden="true" /></Link>}</div><div className="lg:hidden"><MobileNav signedIn={Boolean(session)} /></div></Container></header>; }
